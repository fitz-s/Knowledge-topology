...
- `topology bootstrap claude`: generate Claude local topology wiring and merge
  hook settings.
- `topology bootstrap openclaw`: generate an OpenClaw workspace bundle with
  runtime consume, doctor, evidence capture, lease, writeback, QMD, and skill
  snippets.
...
- P12.3 packages the OpenClaw read/writeback protocol into consumer-local
  wrappers under `.openclaw/topology/`; wrappers resolve fresh topology and
  subject revisions at runtime.
- QMD extra paths generated by the bundle point only at
  `projections/openclaw/file-index.json`, `runtime-pack.json`,
  `runtime-pack.md`, `memory-prompt.md`, and `wiki-mirror/`.
- Hosted OpenClaw services, private workspace writes, memory-wiki/QMD import or
  live validation, and OpenClaw natural-language runtime sanitizer are
  deferred.
